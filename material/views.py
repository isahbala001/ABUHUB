...
def timetable_view(request):
    ...
    # If both parameters are provided, try to find timetable
    if department and level:
        timetables = TimeTable.objects.filter(department__id=int(department), level=int(level))
        
        if timetables.exists():
            timetable = timetables.first()
            return redirect(f'/resource/timetables/{timetable.id}/')
        else:
            # Show error message
            messages.error(request, f"No timetable found for Level {level} in the selected department.")
